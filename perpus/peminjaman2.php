...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($bukuid) || empty($tanggalpeminjaman) || empty($tanggalpengembalian) || empty($statuspeminjaman)) {
        ...
        if ($conn->query($sql) === TRUE) {
            echo '<script>alert("Data peminjaman berhasil ditambahkan!"); window.location.href = "tampil_pinjam2.php";</script>';
        } else {
    ...
    <div class="container">
        <table class="content-table">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                ...
                <label for="tanggalpeminjaman">Tanggal Peminjaman:</label><br>
                <input type="date" name="tanggalpeminjaman" required><br>

                <label for="tanggalpengembalian">Tanggal Pengembalian:</label><br>
                <input type="date" name="tanggalpengembalian" required><br>

                <label for="statuspeminjaman">Status Peminjaman:</label><br>
